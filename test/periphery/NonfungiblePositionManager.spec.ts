...
describe('NonfungiblePositionManager', () => {
  ...
  describe('#burn', () => {
    ...
    it('deletes the token (auto burn when liquidity is cleared)', async () => {
      await nft.connect(other).decreaseLiquidity({ tokenId, liquidity: 100, amount0Min: 0, amount1Min: 0, deadline: 1 })
    ...
    it('removes all liquidity', async () => {
      await expect((await nft.positions(tokenId)).liquidity).to.eq(100)
      await nft.connect(other).decreaseLiquidity({ tokenId, liquidity: 100, amount0Min: 0, amount1Min: 0, deadline: 1 })
      await expect((await nft.positions(tokenId)).liquidity).to.eq(0)
    })
  })
